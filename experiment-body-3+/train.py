...
def get_args():
    ...
    parser.add_argument('--energy_nonlinearity', default='tanh', type=str, help='neural net nonlinearity for engergies')
    ...
    parser.add_argument('--load_obj', default=3,  type=int, help='load saved model for the corresponding system, only works for ModLaNet.')    
    parser.add_argument('--load_epoch', default=0000,  type=int, help='load saved model at epoch k.')    
    parser.add_argument('--end_epoch',   default=3000, type=int, help='end of training epoch')
    parser.add_argument('--use_lr_scheduler', nargs='?', const=True, default=False, help='whether to use lr_scheduler?')
    parser.add_argument('--samples', default=200, type=int, help='the number of sampling trajectories')
    parser.add_argument('--noise', default=0., type=float, help='the noise amplitude of the data')

    # GENERAL SETTINGS
    parser.add_argument('--verbose', dest='verbose', default=False, action='store_true', help='verbose?')
    parser.add_argument('--plot', default=False, action='store_true', help='plot training and testing loss?')
    parser.add_argument('--print_every', default=100,  type=int, help='number of gradient epoch between prints')
    parser.add_argument('--name', default='body', type=str, help='only one option right now')
...
def train_LNN(args):
    ...
    # Example: model-3-body-lnn-hidden_dim-200-end_epoch-3000-noise-0.0-learn_rate-0.001.tar
    start_epoch = 0
    ...
    # generate data
    # naming example: dataset_3_body_modlanet_noise_0.0.npy
    t0 = time.time()
    ...
    for epoch in range(start_epoch, args.end_epoch + 1):
        ...
        if args.verbose and epoch % args.print_every == 0:
            print("epoch {}, train_loss {:.4e}, test_loss {:.4e}".format(epoch, loss.item(), test_loss.item()))
...
def train_HNN(args):
    ...
    nn_model = MLP(input_dim, args.hidden_dim, output_dim, nonlinearity=args.nonlinearity)
    ...
    # load trained models if possible
    # naming example: model-3-body-hnn-hidden_dim-200-end_epoch-3000-noise-0.0-learn_rate-0.001.tar
    start_epoch = 0
    if args.load_epoch > 0:
        path = '{}/model-{}-{}-{}-hidden_dim-{}-end_epoch-{}-noise-{}-learn_rate-{}.tar'.format(args.save_dir, args.obj, args.name,
                                                                           args.model, args.hidden_dim, args.load_epoch,
    ...
    # generate data
    # naming example: dataset_3_body_hnn_noise_0.0.npy
    t0 = time.time()
    ...
    if os.path.exists(filename):
        ...
        print('Start generating dataset.')
        dataset = Dataset(seed=args.seed, obj = args.obj)
...
def train_ModLaNet(args):
    ...
    # init model and optimizer
    model = ModLaNet(obj=args.obj, dim=args.dim, edof=args.dof, device=device, build_computation_tree=BuildComputationTree, transform = 'global',
    ...
    model.to(device=device)
    optim = torch.optim.Adam(model.parameters(), args.learn_rate, weight_decay=1e-4)
    lambda1 = lambda epoch: 1 #if epoch < 2000 else 0.4 if epoch < 4000 else 0.2 if epoch < 6000 else 0.1 if epoch < 9000 else 0.04 if epoch < 10000 else 0.02 if epoch < 11000 else 0.01
    scheduler_enc = LambdaLR(optim, lr_lambda=lambda1)
    ...
    # load trained models if possible
    # Example: model-3-body-modlanet-hidden_dim-50-end_epoch-3000-noise-0.0-learn_rate-0.001.tar
    start_epoch = 0
    if args.load_epoch > 0:
    ...
    test_x = torch.tensor(data['test_x'], requires_grad=True, dtype=torch.float32, device=device)
    test_v = torch.tensor(data['test_v'], requires_grad=True, dtype=torch.float32, device=device)
    test_a = torch.tensor(data['test_ac'], dtype=torch.float32,  device=device)
    ...
    for epoch in range(start_epoch, args.end_epoch + 1):
        ...
        # run test data
        a_hat = model.forward(test_x, test_v)
        test_loss = L2_loss(a_hat, test_a)
        if args.use_lr_scheduler:
            scheduler_enc.step()
        optim.zero_grad()
...
def main():
    ...
    # save model and log
    torch.save({'network_state_dict': model.state_dict(), 'optimizer_state_dict': optim.state_dict(), 'scheduler_state_dict': scheduler.state_dict()}, path)
